if (localStorage.getItem('quantity')) {
    ...
    $('.quantity-input').val(localStorage.getItem('quantity'));
}


// $('.color-quantity').each(function () {
//     const colorId = $(this).data('color-id');
//     const storedValue = localStorage.getItem(`colorQuantity_${colorId}`);
//     if (storedValue) {
//         $(this).val(storedValue);
//     }
// });
if (localStorage.getItem('blankPrice')) {
    $('.d3').val(localStorage.getItem('blankPrice'));
}
if (localStorage.getItem('weight')) {
    $('.e3').val(localStorage.getItem('weight'));
}
if (localStorage.getItem('embroideryType')) {
    $('.g3').val(localStorage.getItem('embroideryType'));
}
if (localStorage.getItem('frontEmbroidery')) {
    $('.h3').val(localStorage.getItem('frontEmbroidery'));
}
if (localStorage.getItem('sideEmbroidery')) {
    if (localStorage.getItem('sideEmbroidery') == 'yes') {
        $('.side_location').show();
    } else {
        $('.side_location').hide();
    }
    $('.i3').val(localStorage.getItem('sideEmbroidery'));
}
if (localStorage.getItem('backEmbroidery')) {
    if (localStorage.getItem('backEmbroidery') == 'yes') {
        $('.back_location').show(); 
    }else{
        $('.back_location').hide(); 
    }
    $('.k3').val(localStorage.getItem('backEmbroidery'));
}
if (localStorage.getItem('sideLocation')) {
    $('.j3').val(localStorage.getItem('sideLocation'));
}
if (localStorage.getItem('backLocation')) {
    $('.l3').val(localStorage.getItem('backLocation'));
}
...
function constantCalculation() {
    ...
    const blankPrice = $('.d3').val();
    const weight = $('.e3').val();
    const embroideryType = $('.g3').val();
    const frontEmbroidery = $('.h3').val();
    const sideEmbroidery = $('.i3').val();
    const backEmbroidery = $('.k3').val();
    const sideLocation = $('.j3').val();
    const backLocation = $('.l3').val();

    // Store values in local storage
    localStorage.setItem('quantity', quantity);
    localStorage.setItem('blankPrice', blankPrice);
    localStorage.setItem('weight', weight);
    localStorage.setItem('embroideryType', embroideryType);
    localStorage.setItem('frontEmbroidery', frontEmbroidery);
    localStorage.setItem('sideEmbroidery', sideEmbroidery);
    localStorage.setItem('backEmbroidery', backEmbroidery);
    localStorage.setItem('sideLocation', sideLocation);
    localStorage.setItem('backLocation', backLocation);


    // $('.color-quantity').each(function () {
    //     const colorId = $(this).data('color-id');
    //     const colorQuantity = $(this).val();
    //     localStorage.setItem(`colorQuantity_${colorId}`, colorQuantity);
    // });
