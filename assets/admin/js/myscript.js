(function($) {
  ...
  $('.select2').select2();
    ...
    //common function for product_name generate base on =CONCAT(brand," ",model," ",fit," ",bill," ","Bill")
    function generateProductName() {

      //select2 select valye text
      var brand = $('#brand_id').find(':selected').text();
      var model = $('#model_number').find(':selected').text();
  
        var fit = $('#fit').val();
        var bill = $('#bill').val();

      
        var product_name = brand + ' ' + model + ' ' + fit + ' ' + bill + ' Bill';
        //capitalize first letter of each word in product_name
        product_name = product_name.replace(/\b\w/g, function(l){ return l.toUpperCase() });
        $('#product_name').val(product_name);
    }

    //call generateProductName function on change of brand, model, fit, bill
    $('#brand_id, #model_number, #fit, #bill').on('change', function() {
        generateProductName();
    });
  ...
  // Display Subcategories & attributes
      $(document).on('change','#cat',function () {
        ...
        if (category_id == 8) {
          ...
          // $('.fit, .bill, .style, .closure, .panel, .model, .brand').show();
        } else {
          // $('.fit, .bill, .style, .closure, .panel, .model, .brand').hide();
