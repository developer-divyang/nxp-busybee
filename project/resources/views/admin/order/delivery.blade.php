        ...
        <div class="product-description">
          <div class="body-area" id="modalEdit">
            ...
            <form id="geniusformdata" action="{{route('admin-order-update',$data->id)}}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row">
                <div class="col-lg-4">
                  <h4 class="heading">{{ __('Total Amount') }} : {{ \PriceHelper::showOrderCurrencyPrice((($data->pay_amount + $data->pending_amount)  * $data->currency_value),$data->currency_sign) }}</h4>
                </div>
                <div class="col-lg-4">
                  <h4 class="heading">{{ \PriceHelper::showOrderCurrencyPrice((($data->pay_amount)  * $data->currency_value),$data->currency_sign) }}</h4>
                </div>
                <div class="col-lg-4">
                  <h4 class="heading">{{ __('Total Amount') }} : {{ \PriceHelper::showOrderCurrencyPrice((( $data->pending_amount)  * $data->currency_value),$data->currency_sign) }}</h4>
                </div>
              </div>
