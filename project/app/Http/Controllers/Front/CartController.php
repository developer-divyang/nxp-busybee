...
class CartController extends FrontBaseController
{
    ...
    public function addcart(Request $request, $id)
    {
        ...
        if ($prod->getProductSizeColor) {
            foreach ($prod->getProductSizeColor as $combination) {
                $color = Color::find($combination->color_id)->color_name;
                $size = Size::find($combination->size_id)->size_name;

                $oldCart = Session::has('cart') ? Session::get('cart') : null;
                $cart = new Cart($oldCart);

                // Check for existing item in cart
                $itemKey = $prod->id . $size . $color;
                // dd($itemKey);
                if (isset($cart->items[$itemKey])) {
                    ...
                    // Check if new quantity exceeds size_qty or stock
                    // if ($prod->size_qty && $newQty > $prod->size_qty) {
                    //     return 0; // Exceeds stock limit
                    // }
                    ...
                    $cart->items[$itemKey]['back_location'] = $request->l3;
        ...
        return redirect()->route('customization-step-1', $prod->slug)->with('success', __('Added TO Cart'));
    }
